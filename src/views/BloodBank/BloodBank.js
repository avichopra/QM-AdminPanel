...
export default class Ambulance extends Component {
  constructor(props) {
    ...
    this.state = {
      count: 0,
      headers: ['Name', 'Address', 'Number', 'Status', 'Action'],
      ambulanceBody: [],
      tableHeaders: ['bloodBankName', 'bloodBankNo', 'bloodBankAddress', 'status', 'action'],
      fields: {
        // ambulanceId: {
        bloodBankName: 1,
        bloodBankNo: 1,
    ...
    return (
      <div className="animated fadeIn">
        <Table
          ...
          addIcon={true}
          addLink={() => this.props.history.push('/home/bloodbank/add')}
          // formLink={item =>
          //   this.props.history.push({
          //     pathname: `/home/bloodbank/edit/?id=${item._id}`,
          //     state: item
          //   })
          // }
          paginationArray={this.state.paginationArray}
          ...
          block={(_id, status) => {
            this.blockBloodBank(_id, status);
          }}
          delete={_id => {
            this.deleteBloodBank(_id);
          }}
        />
      </div>
    );
